...
- [X] T105 Evaluate multi-hypothesis candidate generation that unions unrestricted semantic candidates with bounded candidates from every plausible legal route, without changing trusted retrieval or answer policy
- [X] T106 Record whether ambiguous refugee/asylum keywords should produce both `AufenthG` and `AsylG` weak hints, retaining corpus prior and nationality only as reviewable ranking evidence rather than a hard route decision
