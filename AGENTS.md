...
- Python 3.12 in conda environment `chbot` + Python stdlib JSON/filesystem tooling, deterministic heuristics, optional external LLM batch JSONL contract without default live-service dependency (006-tg-question-answer-evaluation-dataset)
- Python 3.12 in conda environment `chbot` + Python stdlib JSON/filesystem/date handling, existing 006 artifact readers/evaluation patterns, Pydantic validation models, optional LangChain context/structured-output helpers for operator LLM runners, existing retrieval embedding profile/service contracts, pytest; generated JSON/JSONL artifacts under ignored `data/evaluation/` paths (007-legal-question-canonicalization)
